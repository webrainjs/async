...
function isFullscreen(elem): boolean | null {
	if (!elem || !elem.getRootNode) {
		return null
	}

	const document = elem.getRootNode()
	const currentElem = document.fullscreenElement
		|| document.webkitFullscreenElement
		|| document.mozFullScreenElement
		|| document.msFullscreenElement

	return currentElem === document.documentElement
		? true
		: (currentElem ? null : false)
}
...
;['', 'webkit', 'moz', 'ms'].forEach(
	...
	store.destroy = () => {
		['', 'webkit', 'moz', 'ms'].forEach(
			prefix => document.removeEventListener(prefix + 'fullscreenchange', eventHandler, false),
		)
	}
