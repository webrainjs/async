...
import * as externalSrc from 'webrain/src/main/common/index'
import * as externalJs from 'webrain/dist/js/main/common/index'
import * as externalMjs from 'webrain/dist/mjs/main/common/index'
...
describe('common > env > mjs > modules', function () {
	...
	it('external', function () {
		...
		assert.notStrictEqual(externalMjs, external)

		// TODO: uncomment after webrain will completed
		...
		for (const key in external) {
			if (key !== 'default' && key !== '__moduleExports' && key !== '__esModule') {
				assert.ok(external[key] === externalJs[key] && external[key] !== externalMjs[key]
					|| external[key] !== externalJs[key] && external[key] === externalMjs[key])
