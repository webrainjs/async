export function timeToString(time) {
  ...
  time = time / 60 | 0;
  const hour = time;
  return (negative ? '-' : '') + `${hour}:${min.toString().padStart(2, '0')}:${sec.toString().padStart(2, '0')}`;
}
...
export function timeOfDayToString(date) {
  ...
  return [((date.getHours() + 11) % 12 + 1 + '').padStart(2, '0'), ':', (date.getMinutes() + '').padStart(2, '0'), ':', (date.getSeconds() + '').padStart(2, '0'), ' ', date.getHours() > 12 ? 'PM' : 'AM'].join('');
}
...
export function runInContext(win, func, ...args) {
  win[contextPropName] = {
    func,
    args
  };
