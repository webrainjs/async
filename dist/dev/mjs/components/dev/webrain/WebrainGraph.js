...
export let HighlightMode; // region helpers
...
export class WebrainGraph extends ObservableClass {
  ...
  addObjectInfo(object, node, edge) {
  ...
  removeObjectInfo(object, node, edge) {
  ...
  setNode({
    ...
    if (!node) {
      ...
      if (object && object.propertyChanged) {
        let unsubscribed;
        const unsubscribe = object.propertyChanged.hasSubscribersObservable.subscribe(hasSubscribers => {
          if (!hasSubscribers) {
  ...
  setEdge({
    ...
    const toId = to instanceof Node ? to.id : this.getNodeId(to); // if (!fromId && toId) {
    ...
    if (fromId && toId) {
      ...
      const edgeId = calcEdgeId(fromId, toId, type, key, keyType);
...
new CalcObjectBuilder(WebrainGraph.prototype).writable('isEnabled').writable('highlightMode').writable('searchPattern', {
  ...
  factory: () => new WebrainObservableMap(new ObjectMap())
}).readable('edges', {
  factory: () => new WebrainObservableMap(new ObjectMap())
}).calc('visData', connectorFactory({
  name: 'WebrainGraph.Connector.visData' + WebrainGraphObjectsId,
  buildRule: c => c.connect('nodes', b => b.p('nodes')).connect('edges', b => b.p('edges')).connect('highlightMode', b => b.p('highlightMode')).connect('searchPattern', b => b.p('searchPattern'))
}), calcPropertyFactory({
  name: 'WebrainGraph.visData' + WebrainGraphObjectsId,
  dependencies: d => d.invalidateOn(b => b.any(b2 => b2.p('highlightMode', 'searchPattern'), b2 => b2.p('nodes', 'edges').collection().p('updateId'))),
  ...
  *calcFunc(state) {
    ...
    const _calcOpacity = (name, val, itemUpdateId, calcStat) => {
    ...
    for (const groupId in value.groups.nodes) {
      if (Object.prototype.hasOwnProperty.call(value.groups.nodes, groupId)) {
        ...
        const opacity = _calcOpacity(item.name, item.object, itemUpdateId, item.object instanceof CalcProperty ? item.object.timeTotalStat : null);

        const visData = item.getVisData({
          opacity,
          age
        });
        value.nodes[i++] = { ...visData,
          id: groupId
...
async function storeWebrainGraph(object) {
...
export function isWebrainInternalObject(object) {
  ...
  const name = object instanceof CalcProperty && object.state.name || object instanceof Connector && object.connectorState.name;
