...
const init = () => {
  ...
  self.addEventListener('install', event => {
    event.waitUntil(catchLog(caches.open(ASSETS).then(cache => cache.addAll(to_cache)).then(() => {
      self.skipWaiting();
    })));
  });
  self.addEventListener('activate', event => {
    event.waitUntil(catchLog(caches.keys().then(async keys => {
      // delete old caches
  ...
  self.addEventListener('fetch', event => {
    ...
    catchLog(event.respondWith(catchLog(caches.open(`offline${timestamp}`).then(async cache => {
      try {
