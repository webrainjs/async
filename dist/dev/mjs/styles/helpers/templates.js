...
const buttonsReset = {
  [[`input[type='button']`, `input[type='submit']`, `input[type='reset']`, `input[type='color']`, `button`].join(',\n\t')]: [buttonAsDiv],
  [`input[type='file']::-webkit-file-upload-button`]: [{ ...buttonAsDiv,
...
function transition(durationSec, includes = ['opacity', 'background-color', 'color', 'border-color', 'box-shadow', 'fill', 'fill-opacity', 'flood-color', 'flood-opacity', 'lighting-color', 'outline-color', 'stop-color', 'stop-opacity', 'stroke-opacity', 'text-decoration-color', 'text-shadow'], excludes = []) {
  return {
    transition: includes.map(o => `${o} ${durationSec}s`).concat(excludes.map(o => `${o} 0`)).join(', ')
  };
}
