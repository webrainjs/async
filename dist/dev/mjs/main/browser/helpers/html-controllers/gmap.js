...
export function getMarkerClass() {
  if (!Marker) {
    ...
    Marker = (_temp = class Marker extends google.maps.OverlayView {
      constructor({
        map,
        container
      }) {
...
export function gmapPointsToRect(...points) {
  if (!points) {
    return null;
  }
...
export class GMapController {
  ...
  fromPointToLatLng(point) {
    ...
    return this.map.getProjection().fromPointToLatLng(new google.maps.Point((point.x - (mapArea.width - 256) / 2 - pointCenter.x) / zoom + pointCenter.x, (point.y - (mapArea.height - 256) / 2 - pointCenter.y) / zoom + pointCenter.y), true);
  }

  getZoomToFitRect(rect) {
    ...
    const deltaZoom = Math.log2(Math.min((viewArea.width - (this.padding || 0) * 2) / width, (viewArea.height - (this.padding || 0) * 2) / height));
  ...
  getMapPosition(viewAreaPosition) {
    if (!this.viewArea) {
    ...
    const position = this.fromPointToLatLng(new google.maps.Point(point.x + mapAreaCenterX - viewAreaCenterX, point.y + mapAreaCenterY - viewAreaCenterY));
    return position;
  }

  fitRect(rectOrPoint) {
    const zoom = rectOrPoint instanceof google.maps.LatLngBounds ? this.getZoomToFitRect(rectOrPoint) : this.zoomMax || GMapController.ZOOM_MAX;
    this.map.setZoom(zoom);
    const position = this.getMapPosition(rectOrPoint instanceof google.maps.LatLngBounds ? rectOrPoint.getCenter() : rectOrPoint);
