    ...
    function _cssToXPath(xPath: string[], text: string, exclusiveEndIndex: number, writeStartSeparator: boolean = true, braceStart: string = '\0', braceEnd: string = '\0') {
        ...
        while (index < exclusiveEndIndex) {
            ...
            if (ch === ':') {
                ...
                switch (name) {
                    ...
                    case "not":
                        ...
                        if (text[index] === ':') {
                            index++
                            let subFunc = readWord(isNameChar)
                            if (subFunc !== "has") {
                                if (text[index] !== ')') {
                    ...
                    case "contains":
                        ...
                        index++
                        let value = readToChar(')', isNotQuote)
                        xPath.push("[contains(")
            ...
            if (ch === '[') {
                ...
                let attributeName = readWord(isNameChar)
                ch = text[index]
                if (ch === '=') {
                    index++
                    let value = readToChar(']', isNotQuote)
